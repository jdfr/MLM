# MLM: Multiresolution Layer Manufacturing #
...
Toolpaths for several different voxel files generated by MLM (with some tweaking in the case of the tower):
...
Naive configuration with two voxel sizes, both for additive manufacturing:

![additive](.github/example.AutoCAD.additive.png)

Naive configuration with two voxel sizes, the bigger one for additive manufacturing, the smaller one for subtractive manufacturing:
...
Configuration for a similar (thinner) object, with two voxel sizes, both for subtractive manufacturing. The bigger voxel has a custom shape to match a precision spindle, the smaller one is for high-precision laser machining:
...
Special thanks to Ezra Feilden-Irving for taking the time and effort to execute these proofs of concept. These images are used with his permission.
...
## Usage overwiew (very high-level)
